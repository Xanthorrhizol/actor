use crate::{Actor, ActorError, CHANNEL_SIZE, JobSpec, LifeCycle, Mailbox};
...
impl ActorSystem {
    ...
    pub fn restart(&mut self, address_regex: String) {
        if let Err(e) = self.handler_tx.try_send(ActorSystemCmd::Restart(address_regex)) {
                ...
                for address in addresses {
                    match map.entry(address.to_string()) {
                        std::collections::hash_map::Entry::Occupied(mut entry) => {
                            let _ = entry.get_mut().2.send(()).await;
                            entry.remove_entry();
